...
class ArrayList {
    ...
    public int search(String nameGame) {
        int posicao = 0;

        for(Cellule i = first.next; i != null; i = i.next) {
            if(i.game.getName().equals(nameGame)) {
                i = null;
            } else {
                posicao++;
            }
...
class Hash {
    private int size;
    private ArrayList[] listGames;

    Hash() {
        size = 21;
        listGames = new ArrayList[size];

        for(int i = 0; i < size; i++)
            listGames[i] = new ArrayList();
    }
    ...
    public void insert(Game game) throws IOException, ParseException {
        int function = hash(game.getName());
        listGames[function].insertEnd(game);
    }

    // Metodo de Remover Elementos no Hash
    public void remove(String name) throws Exception {
        int function = hash(name);
        int position = listGames[function].search(name);
        listGames[function].remove(position);
    }
    ...
    public boolean search(String name) {
        ...
        int listPositon = listGames[position].search(name);

        if(listPositon != -1) {
            resp = true;
            System.out.println("Posicao Hash: " + position + " Posicao Lista: " + listPositon);
        } 
...
class TP05Q07 {
    ...
    public static Hash readAndSavePiubIn(Game listGames[]) throws Exception {
        String pubIn = "";
        Hash hash = new Hash();
    ...
    // Metodo para Inserir, Remover e Pesquisar Elementos
    public static void elementManipulation(Hash hash, Game[] listGames) {
        String entry = "";
    ...
    public static void main(String[] args) throws Exception {
        ...
        Hash hash = readAndSavePiubIn(database);
    	elementManipulation(hash, database);
    }
}