...
class No {
	...
	public void setLevel() {
		level = 1 + Math.max(getLevel(left), getLevel(right));
	}
...
class Tree {
	...
	private No remove(String name, No i) throws Exception {
		if(i == null) {
			throw new Exception("Erro ao Remover");
		} else if (name.compareTo(i.game.getName()) < 0) {
			i.left = remove(name, i.left);
		} else if (name.compareTo(i.game.getName()) > 0) {
			i.right = remove(name, i.right);
		} else if (i.left == null) {
			i = i.right;
		} else if (i.right == null) {
			i = i.left;
		} else {
			i.left = beggerLeft(i, i.left);
		}

		return balance(i); 
	}

	private No beggerLeft(No i, No j) {
		if(j.right == null) {
			i.game = j.game;
			j = j.left;
		} else {
			j.right = beggerLeft(i, j.right);
		}

		return j;
	}
	
	// Metodo de Balancear a Arvore
	private No balance(No i) throws Exception {
		if(i != null) {
			int factor = No.getLevel(i.right) - No.getLevel(i.left);
			
			// Arvore Balanceada
			if(Math.abs(factor) <= 1) {
				i.setLevel();  
			}  else if (factor == 2) {
				int sonFactor = No.getLevel(i.right.right) - No.getLevel(i.right.left);
							
				if(sonFactor == -1) {
					i.right = rotationRight(i.right);
				}

				i = rotationLeft(i);
			} else if (factor == -2){
				int sonFactor = No.getLevel(i.left.right) - No.getLevel(i.left.left);

				if(sonFactor == 1) {
					i.left = rotationLeft(i.left);
				}
	...
	// Metodo de Rotacionar para Esquerda os Elementos
	private No rotationRight(No i) {
		No noLeft = i.left;
	...
	// Metodo de Rotacionar para Direia os Elementos
	private No rotationLeft(No i) {
		No noRight = i.right;
	...
	// Metodo de Pesquisar Elementos na Arvore
