...
class ArrayList {
    private Game[] games, removedGames;
    private int currentPosition, removedGamesPosition; 
    ...
    public void sort() {
        // Alterar a Array ignorando a posicao zero
        Game[] tmp = new Game[games.length+1];
        for(int i=0; i<currentPosition; i++) {
            tmp[i+1] = games[i].clone();
        }

        // Construção do HeapSort
        for(int tamHeap = 2; tamHeap <= currentPosition; tamHeap++) {
            build(tmp, tamHeap);
        }

        // Ordenacao propriamente dita
        int tamHeap = currentPosition;
        while(tamHeap > 1) {
            swap(tmp, 1, tamHeap--);
            rebuild(tmp, tamHeap);
        }

        // Alterar a Array para voltar a posicao zero
        for(int i = 0; i < currentPosition; i++) {
            games[i] = tmp[i+1];
        }
    }
    ...
    public void swap(Game tmp[], int i, int j) {
        Game temp = tmp[i].clone();
        tmp[i] = tmp[j].clone();
        tmp[j] = temp.clone();
    }

    // Metodo de Construir o HeapSort
    private void build(Game tmp[], int tamHeap) {
        for(int i = tamHeap; i>1 ; i /= 2) {
            Date dateTmpI = tmp[i].getReleaseDate();
            Date dateTmpI2 = tmp[i/2].getReleaseDate(); 
            if(dateTmpI.compareTo(dateTmpI2) > 0) {
                swap(tmp, i, i/2);
            } 
        }
    }

    // Metodo de Reconstruir o HeapSort
    private void rebuild(Game tmp[], int tamHeap) {
        int i=1;
        while(i <= tamHeap/2) {
            int son = getGreatestSon(tmp, i, tamHeap);
            Date dateTmpI = tmp[i].getReleaseDate();
            Date dateTmpI2 = tmp[son].getReleaseDate();
            if(dateTmpI.compareTo(dateTmpI2) < 0) {
                swap(tmp, i, son);
                i = son;
            } else {
    ...
    // Metodo de Retorna o maior Filho
    private int getGreatestSon(Game tmp[], int i, int tamHeap) {
        int son;
        if(2*i == tamHeap || tmp[2*i].getReleaseDate().compareTo(tmp[2 * i + 1].getReleaseDate()) > 0) {
            son = 2*i;
        } else {
            son = 2*i+1;
        }
...
class TP03EX05 {
    ...
    public static void main(String[] args) throws Exception{
        ...
        ArrayList listGames = new ArrayList(pubInGames.length);
