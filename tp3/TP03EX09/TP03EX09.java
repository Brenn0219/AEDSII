...
class ArrayList {
    ...
    // Algoritmo de Ordenacao por MergeSort
    public void sort() {
    ...
    private void sort(int left, int right) {
        if(left < right) {
            ...
            sort(left, middle);
            sort(middle+1, right);
            intercalary(left, middle, right);
        }
    }

    // Intercalacao dos elementos dos SubArrays
    private void intercalary(int left, int middle, int right) {
        int i, j, k;
        ...
        for(j=0; j<n2; j++) {
            a2[j] = games[middle+j+1];
        }

        // Sintinela no final dos dois Arrays (Upvotes)
        a1[i] = a2[j] = new Game();
        a1[i].setUpvotes(Double.MAX_VALUE);
        a2[j].setUpvotes(Double.MAX_VALUE);
