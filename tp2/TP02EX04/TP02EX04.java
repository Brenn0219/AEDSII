...
class TP02EX04 {
    ...
    public static void binarySearch(Game[] pubInsGames) {
        String pubIn = "";
        boolean result = false;
        int left = 0, right = pubInsGames.length - 1;
        
        String names[] = new String[pubInsGames.length];
        for(int i=0; i<pubInsGames.length; i++) {
            names[i] = pubInsGames[i].getName();
        }

        // Arry Sort feito com Insercao
        for (int i = 1; i < names.length; i++) {
            String tmp = names[i];
            int j = i - 1;

            while ((j >= 0) && ((names[j].compareTo(tmp)) > 0)) {
                names[j + 1] = names[j];
                j--;
            }
            names[j + 1] = tmp;
        ...
        do {
            ...
            if(theEnd(pubIn) != true) {
                while(left <= right) {
                    ...
                    String auxiliaryString = new String(names[middle]);
                    int x =  pubIn.compareTo(auxiliaryString);
                ...
                left = 0;
                right = pubInsGames.length-1;
                result = false;
            }
        }while(theEnd(pubIn) != true);
    }

    // Metodo para ordenar por nome - Insercao
    public static String[] insertion(Game[] games) {
