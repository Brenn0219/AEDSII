...
class Queue {
    ...
    // Metodo para contar os avgpts da Fila Circular
    public void avgptQueueFormated() {
        double result = 0;

        Cellule start = first.next;

        while (start != last.next) {
            result += start.game.getAvgPt();
            start = start.next;
        }
        ...
        long averageAvg = Math.round(result/numAvg);
    	System.out.println(averageAvg);
    }
...
class TP04EX03 {
    ...
    public static void queueFlexible(Queue games, Game[] listGames) throws Exception {
        int size = sc.nextInt();
        String pubIn = "";
        Game game;
        ...
        for(int i=0; i<size; i++) {
            ...
            if(pubIn.equals("I")) {
                ...
                System.out.println("I " + idLine);
                game = searchGameInGameList(listGames, idLine);
                games.insert(game);
            } else if (pubIn.equals("R")){
                System.out.println("R");
                System.out.println("(R) " + games.remove().getName());
            }
        }
    ...
    public static void main(String[] args) throws IOException, ParseException, Exception{
        ...
        Queue listGames = new Queue();
        
        for(int i = 0; i < pubInGames.length; i++) {
            listGames.insert(pubInGames[i]);
        }
        
        queueFlexible(listGames, pubInGames);
    }   
}